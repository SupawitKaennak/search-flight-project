...
/**
 * สร้าง URL สำหรับรูปภาพจาก backend API
 * ใช้ NEXT_PUBLIC_API_URL สำหรับการดึงรูปภาพจาก backend
 * 
 * @param imagePath - path ของรูปภาพ (เช่น '/airlines/thai-airways.png', '/bangkok.jpg', '/placeholder.svg')
 * @returns URL เต็มสำหรับรูปภาพจาก backend
 * 
 * @example
 * getImageUrl('/airlines/thai-airways.png') // 'http://localhost:3001/api/airlines/thai-airways.png'
 * getImageUrl('/bangkok.jpg') // 'http://localhost:3001/api/bangkok.jpg'
 * getImageUrl('/placeholder.svg') // 'http://localhost:3001/api/placeholder.svg'
 */
export function getImageUrl(imagePath: string): string {
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'
  // ลบ leading slash ถ้ามี เพื่อป้องกัน double slash
  const cleanPath = imagePath.startsWith('/') ? imagePath.slice(1) : imagePath
  // รวม URL โดยใช้ trailing slash จาก apiUrl ถ้ามี
