...
export interface FlightDataSource {
  analyzeFlightPrices(params: FlightSearchParams, signal?: AbortSignal): Promise<FlightAnalysisResult>
  getFlightPrices?(params: FlightPriceParams, signal?: AbortSignal): Promise<FlightPrice[]>
}
...
class RealFlightDataSource implements FlightDataSource {
  async analyzeFlightPrices(
    ...
    return await flightApi.analyzeFlightPrices(request, signal)
  }

  async getFlightPrices(params: FlightPriceParams, signal?: AbortSignal): Promise<FlightPrice[]> {
    return await flightApi.getFlightPrices({
      ...params,
      stops: params.stops || 'all',
    }, signal)
  }
