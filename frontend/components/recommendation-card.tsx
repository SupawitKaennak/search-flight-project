          ...
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            <Popover>
              ...
              <PopoverContent
                align="center"
                sideOffset={8}
                className="w-[92vw] max-w-md sm:w-96 px-3 sm:px-4 py-3 sm:py-4"
              >
                <div className="space-y-3 sm:space-y-4">
                  <div className="font-semibold text-sm mb-1 sm:mb-2">
                    {'เปรียบเทียบกับ Season อื่นๆ'}
                  </div>
                  
                  {/* Current Season */}
                  <div className="p-3 sm:p-3.5 bg-background rounded-lg border">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      ...
                      <div 
                        className={`text-sm sm:text-base font-bold ${currentSeason === 'low' ? 'text-green-600' : currentSeason === 'high' ? 'text-red-600' : 'text-blue-600'}`}
                      >
                  ...
                  {/* Other Seasons */}
                  <div className="space-y-2 max-h-[260px] sm:max-h-none overflow-y-auto pr-1 sm:pr-0">
                    {seasonComparisons.map((comp) => {
                      ...
                      return (
                        <div key={comp.type} className="p-3 bg-background rounded-lg border">
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <span className="text-xs sm:text-sm font-medium">{comp.name}</span>
                            <div className={`text-xs sm:text-sm font-bold ${compColor}`}>
                              {'฿'}{comp.price > 0 ? comp.price.toLocaleString() : '-'}
                            </div>
                          </div>
                          {/* ✅ Always show months if available */}
                          {comp.months && (
                            <div className="flex items-center justify-between gap-2 text-[11px] sm:text-xs mb-1">
                              ...
                              {comp.price > 0 && comparisonBasePrice > 0 && (
                                <span className={`${isCheaper ? 'text-green-600' : 'text-red-600'} font-semibold whitespace-nowrap`}>
                ...
                <div className="p-5 bg-background rounded-lg border">
                  ...
                  {seasonComparisons.length > 0 ? (
                    <div className="space-y-2">
                      {seasonComparisons.map((comp) => {
                        // ✅ ใช้ percentage และ difference ที่คำนวณไว้แล้วจาก comparisonBasePrice (current season price)
                        // เพื่อให้การเปรียบเทียบสอดคล้องกับส่วนอื่นๆ
                        const isCheaper = comp.difference < 0
