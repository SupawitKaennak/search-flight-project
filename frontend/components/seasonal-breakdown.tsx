...
export function SeasonalBreakdown({ seasons: propSeasons, recommendedPeriod, destination, priceComparison, searchParams, flightPrices }: SeasonalBreakdownProps) {
  ...
  // ✅ ใช้เดือนของวันที่เลือก (searchParams.startDate) แทนเดือนปัจจุบัน
  // เพื่อให้สอดคล้องกับ recommendedPeriod.season ที่ใช้ในคำแนะนำ
  const getTargetMonth = (): number => {
    if (searchParams?.startDate) {
      // ใช้เดือนของวันที่เลือก
      const selectedDate = searchParams.startDate instanceof Date 
        ? searchParams.startDate 
        : new Date(searchParams.startDate)
      return selectedDate.getMonth()
    }
    // Fallback: ใช้เดือนปัจจุบันถ้าไม่มีวันที่เลือก
    return new Date().getMonth()
  }
  const currentMonth = getTargetMonth()
  ...
  // ✅ ใช้ recommendedPeriod.season เป็นหลัก (ถ้ามี) เพื่อให้สอดคล้องกับคำแนะนำ
  // Fallback: ใช้ monthSeasonMap[currentMonth] ถ้าไม่มี recommendedPeriod.season
  const currentSeason = recommendedPeriod?.season || monthSeasonMap[currentMonth] || 'normal'
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-0">
      ...
      {/* Note: Recommendation card is shown in PriceAnalysis component, so we skip it here */}
      {/* But we still use recommendedPeriod.season for consistency */}
      {false && recommendedPeriod && (() => {
