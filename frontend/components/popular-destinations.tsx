...
import { formatDateToUTCString, getImageUrl } from '@/lib/utils'
...
// ใช้ getImageUrl เพื่อดึงรูปภาพจาก backend ผ่าน NEXT_PUBLIC_API_URL
const getProvinceImage = (provinceValue: string): string => {
  ...
  const imagePath = imageMap[provinceValue] || '/placeholder.svg'
  // ใช้ NEXT_PUBLIC_API_URL สำหรับดึงรูปภาพจาก backend
        ...
        const destinationsDataPromises = stats.popularDestinations
          ...
          .map(async (dest, index) => {
            ...
            return {
              ...
              count: dest.count,
              provinceValue,
              image: getProvinceImage(provinceValue),
        ...
        {destinations.map((dest) => {
          ...
          >
            <div className="relative h-48 bg-muted rounded-t-xl">
              <img 
                src={dest.image || getImageUrl("/placeholder.svg")} 
                alt={dest.destinationName || dest.destination}
                className="w-full h-full object-cover rounded-t-xl"
                onError={(e) => {
                  const target = e.target as HTMLImageElement
                  const placeholderUrl = getImageUrl('/placeholder.svg')
                  // Prevent infinite loop - only set if not already set
                  if (target.src !== placeholderUrl && !target.dataset.failed) {
