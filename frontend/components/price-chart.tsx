...
export function PriceChart({ data, tripType, selectedDate, passengerCount }: PriceChartProps) {
  ...
  // ใช้เฉพาะราคาที่มากกว่า 0 (ไม่นับจุด synthetic วันที่เลือกที่ไม่มีข้อมูล)
  const positivePrices = chartData.map(d => d.price).filter(p => p > 0)
  const lowestPrice = positivePrices.length > 0 ? Math.min(...positivePrices) : 0

  // หาวันที่ที่มีราคาต่ำสุด (เฉพาะจุดที่มีราคาจริง > 0)
  const lowestPriceDates = chartData
    .filter(d => d.price > 0 && d.price === lowestPrice)
    .map(d => d.startDate)
  ...
  // ฟังก์ชันแปลงวันที่ไทย (เช่น "11 ธ.ค.") เป็น Date object
  // ใช้ปีจาก selectedDate หรือปีปัจจุบัน
  const parseThaiDateToDate = (dateStr: string): Date | null => {
  ...
  // ฟังก์ชันแปลง Date เป็นรูปแบบ ว/ด/ป (เช่น "11/12/2024")
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const departureDate = parseThaiDateToDate(data.date)
      
      // คำนวณวันที่กลับ: ใช้ returnDate ถ้ามี หรือคำนวณจาก departureDate + duration
      let returnDate: Date | null = null
      if (tripType === 'round-trip') {
        if (data.returnDate) {
          returnDate = parseThaiDateToDate(data.returnDate)
        } else if (departureDate && data.duration && data.duration > 0) {
          // คำนวณจาก duration (วัน)
      ...
      // ถ้าไม่มีข้อมูลราคา ไม่แสดง tooltip
      if (price === 0 && !data.isSelected) {
        return null
      }
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <p className="text-lg sm:text-xl font-bold text-blue-600">
                {price > 0 ? `฿${price.toLocaleString()}` : 'ไม่มีข้อมูล'}
              </p>
            </div>
            ...
            {departureDate && (
              <div>
                <p className="text-xs text-gray-500 mb-1">วันที่ออกเดินทาง</p>
                <p className="text-sm sm:text-base font-semibold text-gray-800">
                  {formatDateToDDMMYYYY(departureDate)}
                </p>
              </div>
            )}

            {/* วันที่กลับ (ถ้าเป็นไป-กลับ) */}
            {tripType === 'round-trip' && returnDate && (
              <div>
                <p className="text-xs text-gray-500 mb-1">วันที่กลับ</p>
                <p className="text-sm sm:text-base font-semibold text-gray-800">
                  {formatDateToDDMMYYYY(returnDate)}
                </p>
              </div>
            )}
          </div>
  ...
  // คำนวณ margin และ fontSize ตามขนาดหน้าจอ
  const chartMargin = isMobile 
    ? { top: 20, right: 10, left: 0, bottom: 100 }
    : { top: 30, right: 40, left: 10, bottom: 80 }
  ...
  return (
    <div className="w-full max-w-full overflow-x-auto">
      <div className="min-h-[350px] sm:min-h-[400px] w-full" style={{ minWidth: isMobile ? '600px' : '100%' }}>
        <ChartContainer config={chartConfig}>
          <ResponsiveContainer width="100%" height={isMobile ? 350 : 400}>
            <AreaChart data={transformedData} margin={chartMargin}>
              ...
              <XAxis
                ...
                textAnchor="end"
                height={xAxisHeight}
                interval={isMobile ? Math.max(0, Math.floor(transformedData.length / 8)) : 0}
                tick={(props: any) => {
                  ...
                  return (
                    <text
                      ...
                      textAnchor="end"
                      transform={`rotate(${xAxisAngle}, ${props.x}, ${props.y})`}
                    >
                      {dateStr}
                    </text>
                  )
            ...
            <YAxis
              ...
              tickFormatter={(value) => `THB ${(value / 1000).toFixed(0)}k`}
