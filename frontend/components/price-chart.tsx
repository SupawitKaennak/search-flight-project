...
import { Area, AreaChart, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceLine, Line, Label } from 'recharts'
...
export function PriceChart({ data, tripType, selectedDate, passengerCount }: PriceChartProps) {
  ...
  // ใช้เฉพาะข้อมูลที่มี price > 0 เพื่อความแม่นยำ
  const validData = transformedData.filter(d => d.price > 0)
  ...
  if (validData.length >= 2) {
    hasRegression = true
    // คำนวณ regression coefficients: y = mx + b
    // x = index ใน transformedData (0, 1, 2, ...), y = price
    ...
    validData.forEach((d) => {
      // หา index ของจุดนี้ใน transformedData
      const x = transformedData.findIndex(td => td.date === d.date)
      const y = d.price
      sumX += x
      sumY += y
      sumXY += x * y
    ...
    regressionSlope = denominator !== 0 ? (n * sumXY - sumX * sumY) / denominator : 0
    regressionIntercept = (sumY - regressionSlope * sumX) / n
    
    // เพิ่ม regressionLine เข้ากับ transformedData ทุกจุด
    transformedData.forEach((d, index) => {
      // คำนวณ regression value สำหรับทุกจุด (แม้จุดนั้นไม่มีข้อมูล)
      const regressionValue = regressionSlope * index + regressionIntercept
      ;(d as any).regressionLine = Math.round(regressionValue)
    })
  }
  return (
          ...
          <AreaChart data={transformedData} margin={{ top: 30, right: 40, left: 10, bottom: 80 }}>
            ...
            <ChartTooltip 
              content={<ChartTooltipContent 
                formatter={(value: any, name: any, props: any) => {
                  // ✅ แสดง regression line ต่างจากราคาปกติ
                  if (name === 'regressionLine' || name === 'เส้นแนวโน้ม') {
                    return [
                      `฿${Number(value).toLocaleString()}`,
                      'เส้นแนวโน้ม (Linear Regression)'
                    ]
                  }
                  ...
                  const pricePerPerson = passengerCount && passengerCount > 1 
                    ? Math.round(Number(value) / passengerCount) 
                    : null
                  
                  if (tripType === 'one-way') {
                    // One-way: แสดงแค่ราคา (ไม่แสดง label ซ้ำ)
                    return [
                      `฿${Number(value).toLocaleString()}${pricePerPerson ? ` (รวม ${passengerCount} คน)` : ''}`,
                      pricePerPerson ? `฿${pricePerPerson.toLocaleString()} ต่อคน` : 'ราคาเที่ยวเดียว'
                    ]
                  } else {
                    ...
                    if (returnDate && startDate) {
                      return [
                        `฿${Number(value).toLocaleString()}${pricePerPerson ? ` (รวม ${passengerCount} คน)` : ''}`,
                        pricePerPerson ? `฿${pricePerPerson.toLocaleString()} ต่อคน` : `ไปวันที่ ${startDate} - กลับวันที่ ${returnDate}${duration > 0 ? ` (รวม ${duration} วัน)` : ''}`
                      ]
                    }
                  return [
                    `฿${Number(value).toLocaleString()}${pricePerPerson ? ` (รวม ${passengerCount} คน)` : ''}`,
                    pricePerPerson ? `฿${pricePerPerson.toLocaleString()} ต่อคน` : (returnDate ? `กลับ ${returnDate}` : 'ราคาไป-กลับ')
                  ]
                  }
                }}
            ...
            {hasRegression && (
              <Line
                ...
                connectNulls={false}
                label={(props: any) => {
                  // แสดง label ที่จุดสุดท้าย
                  const isLastPoint = props.index === transformedData.length - 1
                  if (isLastPoint) {
                    return (
                      <text
                        x={props.x}
