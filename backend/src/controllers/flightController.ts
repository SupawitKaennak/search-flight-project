...
export async function getFlightPrices(
  ...
  try {
    ...
    if (!isDirectAirportCode && (originAirportCode === 'BKK' || origin.toLowerCase() === 'bangkok' || origin.toLowerCase().includes('กรุงเทพ'))) {
      ...
      console.log(`[FlightController] Bangkok origin detected (city/province), querying both BKK and DMK`);
    } else if (isDirectAirportCode && originAirportCode === 'BKK') {
      console.log(`[FlightController] BKK selected directly, using only BKK (not including DMK)`);
    }
    ...
    // Helper: parse price from price_text (e.g. "฿1,043" -> 1043) when price/base_price are 0 or missing
    const parsePriceText = (text: string | null | undefined): number | null => {
    ...
    const flightPrices = flightRecords.map((fp: any) => {
      ...
      // If price is 0 or missing, use base_price or parse price_text (Airpaz CSV has price_text)
      let unitPrice = Number(fp.price);
      if (!unitPrice && fp.base_price != null) unitPrice = Number(fp.base_price);
      if (!unitPrice && fp.price_text) {
        const parsed = parsePriceText(fp.price_text);
        if (parsed != null) unitPrice = parsed;
      }

      const adultPrice = unitPrice * passengers.adults;
      const childPrice = unitPrice * passengers.children * 0.75; // 25% discount for children
      const infantPrice = unitPrice * passengers.infants * 0.1; // 90% discount for infants (lap infants)
