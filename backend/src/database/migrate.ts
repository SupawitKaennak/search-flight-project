...
async function runMigration(migration: Migration): Promise<void> {
  try {
    // Try multiple paths for migrations (development vs production)
    const possiblePaths = [
      join(MIGRATIONS_DIR, migration.file), // Development: src/database/migrations
      join(__dirname, 'migrations', migration.file), // Production: dist/database/migrations
      join(process.cwd(), 'src', 'database', 'migrations', migration.file), // Alternative
      join(process.cwd(), 'dist', 'database', 'migrations', migration.file), // Alternative
    ];
    ...
    if (!sql) {
      throw new Error(`Migration file not found: ${migration.file}. Tried: ${possiblePaths.join(', ')}`);
    }
...
async function migrate(direction: 'up' | 'down' = 'up'): Promise<void> {
  try {
    ...
    // Don't exit if called from server startup (pool will be reused)
    ...
    // Only close pool if this is run as standalone script
    if (require.main === module) {
      await pool.end();
    }
  }
