...
```bash
# ที่ root: search-flight-project/.env
NEXT_PUBLIC_OPENWEATHERMAP_API_KEY=your_actual_api_key_here
```

2. **⚠️ ต้อง Rebuild frontend image:**
```bash
...
**⚠️ หมายเหตุสำคัญ:**
- Next.js ต้องการ `NEXT_PUBLIC_*` variables ที่ **build time** ไม่ใช่ runtime
- ต้อง **rebuild image** เมื่อเปลี่ยน API key (ไม่ใช่แค่ restart)
- API key ต้องอยู่ใน **root `.env`** file (docker-compose อ่านอัตโนมัติ)
- ถ้าไม่มี API key, Weather Display จะแสดงข้อความแจ้งเตือนแทนที่จะซ่อนไป

**ดูรายละเอียดเพิ่มเติม:** `WEATHER-API-KEY-FIX.md`
