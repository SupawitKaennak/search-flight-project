...
```bash
# Start Database (Docker) 🐳 — ไฟล์อยู่ที่ root โปรเจค
docker-compose up -d
...
**PostgreSQL แบบติดตั้งเอง (หรือเชื่อมจาก host ไป Docker ที่ port 5432):**
```bash
...
SELECT COUNT(*) FROM search_statistics;
```
...
TRUNCATE TABLE flight_prices, route_price_statistics, search_statistics, price_statistics, routes, airlines, airports CASCADE;
```
...
# นำเข้าเที่ยวบินจาก CSV
npm run import-flights-from-csv

# นำเข้า AirPaz / เที่ยวบินระหว่างประเทศ
...
Backend:
  ...
  - Data: backend/data/
  - Docker: docker-compose.yml (ที่ root โปรเจค)
...
npm run import-flights-from-csv # Import flights from CSV
npm run import-airports        # Import airports from CSV
```
